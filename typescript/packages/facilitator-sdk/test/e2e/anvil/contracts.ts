...
export async function deployTestContracts(
  ...
  // Use absolute path for reliability across different execution contexts
  const contractsDir = "/Users/jolestar/opensource/src/github.com/nuwa-protocol/x402-exec/contracts/out";

  // Deploy MockUSDCWithSignatureValidation (ERC20 + EIP-3009)
  const tokenBytecode = readFileSync(
    join(contractsDir, "MockUSDCWithSignatureValidation.sol/MockUSDCWithSignatureValidation.json"),
    "utf-8"
  );
  ...
  const routerBytecode = readFileSync(
    join(contractsDir, "SettlementRouter.sol/SettlementRouter.json"),
    "utf-8"
  );
  ...
  const routerAddress = routerReceipt.contractAddress as Address;

  console.log(`[Deploy] SettlementRouter at ${routerAddress}`);

  // Deploy TransferHook
  const hookBytecode = readFileSync(
    join(contractsDir, "TransferHook.sol/TransferHook.json"),
    "utf-8"
  );
  ...
  const hookAddress = hookReceipt.contractAddress as Address;
