...
export type ExecuteStatus = 'idle' | 'preparing' | 'signing' | 'submitting' | 'confirming' | 'success' | 'error';
...
export function useX402Execute(): UseX402ExecuteReturn {
  ...
  const execute = async (params: ExecuteParams, network: Network): Promise<ExecuteResult> => {
    ...
    try {
      ...
      setStatus('signing');
      const executeResult = await client.execute(params, true); // waitForConfirmation = true
